...
if [ ${TIDY_CMD_ORIGINAL:-0} == 0 ] && [ "$TIDY" ]; then echo "debug: The environment variable TIDY is not set, but searched ($TIDY_LIST) for a usable, up to date version.  Picked '$TIDY_TRIED' which resolved to '$TIDY_CMD'. Version check status: '$TIDY_OK' (0 = unuseable, 1 = usable)."; fi;
...
echo "debug: Using '$TIDY' which resolves to '$TIDY_CMD'."; 

echo "debug: Checking documentation for HTML errors."
...
if [ "$PRINTED_TIDY_BANNER" != "Yes" ]; then
  echo "debug: HTML documentation check complete, no errors.";
else
  exit 1;
fi;
